...
- Headless feel check: `godot --headless --fixed-fps 60 --path runtime -s res://tests/maya_feel_check.gd` (exit 0 = pass, 54 checks: harness tick guard, run/jump/buffer, mantle, crouch/prone, T-020 tint + slot selection + Identidad hue band, pilot smoke). Green on Godot 4.2.2, 4.3, 4.4.1, 4.5, 4.6 and 4.7 headless — the harness sets inputs from the idle frame so `is_action_just_pressed` lands in the same physics tick on every version (4.3+ lag a tick when driven from `physics_frame`).
