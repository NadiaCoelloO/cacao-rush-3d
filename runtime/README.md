...
- **T-020 greybox cream/tan** (2D PR #12, recolor only): while crouching / crawling `player_maya.gd` overrides the `hero_grey` outfit colour at runtime — crouch **RGB 138,99,65**, crawl **RGB 148,110,76** (average opaque outfit of the 2D `crouch-1` / `crawl-1` PNGs). Only the `Maya_Body` slot of `hero_grey.glb` (Assets @ 402121f: idle cream body, brown hair, tan pack, unlit Emission) is tinted — hair / pack keep the idle materials, as in the 2D recolor; the tint goes through `emission` on that unlit export, through `albedo_color` on plain materials (CSG fallback, tinted whole). Standing puts the untouched materials back. No crouch mesh (high-poly HOLD). **Identidad PASA** (greybox, 2026-09-26): tints stay in the ~28° hue band (crouch 27.9°, crawl 28.3°), never olive.
- Headless feel check: `godot --headless --fixed-fps 60 --path runtime -s res://tests/maya_feel_check.gd` (exit 0 = pass, 51 checks: run/jump/buffer, mantle, crouch/prone, T-020 tint + Identidad hue band, pilot smoke).
